// ---------------------------------------------------------------------------
// SP Portal — Frontend data layer: DEMO/MOCK implementation (v3.1).
//
// localStorage-backed MOCK of the FastAPI backend, so the UI is fully runnable
// without a server — used for the PORTFOLIO/demo build (data contoh, bukan data
// asli). Selected at build time by `./api.ts` when DEMO_MODE is on. Core model: a
// RegistrationSession groups many Participant rows entered directly (no separate
// "pendata"). The REAL backend counterpart lives in `./api.real.ts`.
// ---------------------------------------------------------------------------
...
// Hanya seed data contoh saat mode DEMO. Di mode REAL, modul ini tetap ter-import
// (untuk shortCode) tetapi tidak boleh menyentuh localStorage.
if (DEMO_MODE) seed();
...
export function getSetupStatus(): Promise<{ only_default_admin: boolean }> {
  return delay({ only_default_admin: true });
}
