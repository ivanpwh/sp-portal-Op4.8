...
| `GET /api/participants/public` | publik | — | — | `services.publicParticipants()`: `PublicSpIndukGroup[]` — `{ induk, participants: { full_name, nickname, sp_code, whatsapp_number, email }[] }[]`, hanya peserta `will_attend` | — | `getPublicParticipants()` |
